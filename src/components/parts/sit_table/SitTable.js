...
const DinnerTableSvg = ({fill="#ffffff", width="800px", height="800px"}) => {

    return (<svg fill={fill} version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 214.539 214.539" width={width} height={height}
            xmlSpace="preserve">
        <g>
            <g>
...
export const SitTable = ({tableNumber, reservations_table, onClick}) => {
        ...
        <div id={table_id} className={styles.container}>
            ...
            {tableNumber !== 0 && (
                <div className={styles.wrapper} onClick={onClick}>
                    <DinnerTableSvg width="100px" height="100px" />
                    <div className={styles.reservations_count_wrapper}>
                        <div className={styles.table_number}>No. {tableNumber}</div>
                        <div className={styles.reservations_count}>{reservations_table.length}</div>
                    </div>
                </div>
            )}
            {/* {popover} */}
